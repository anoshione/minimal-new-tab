...
document.addEventListener("DOMContentLoaded", () => {
  ...
  allWidgets = Array.from(new Set([...allWidgets, ...defaultSettings.availableWidgets]));
        ...
        widgetRows.forEach((row) => {
          ...
          if (input && input.checked) {
            selectedWidgets.push(
              input.getAttribute("data-widget"),
            );
          }
        });
        settings_obj[key] = selectedWidgets;
      } else if (key === "availableWidgets") {
        const currentAvail = settings.availableWidgets || defaultSettings.availableWidgets;
        settings_obj[key] = Array.from(new Set([...currentAvail, ...defaultSettings.availableWidgets]));
...
function updateOptionsPixelArtColor() {
  const isDark = document.body.classList.contains("dark");
  const darkInput = document.getElementById("pixelArtColorDark");
  const lightInput = document.getElementById("pixelArtColorLight");
  const color = isDark ? (darkInput?.value || "#cccccc") : (lightInput?.value || "#b04288");
  document.body.style.setProperty("--pixel-art-color", color);
  document.documentElement.style.setProperty("--pixel-art-color", color);
  document.body.style.setProperty("--color-accent", color);
  document.documentElement.style.setProperty("--color-accent", color);

  const contrastColor = getContrastTextColor(color);
  document.body.style.setProperty("--color-accent-text", contrastColor);
  document.documentElement.style.setProperty("--color-accent-text", contrastColor);
}

document.getElementById("pixelArtColorDark")?.addEventListener("input", updateOptionsPixelArtColor);
document.getElementById("pixelArtColorLight")?.addEventListener("input", updateOptionsPixelArtColor);
